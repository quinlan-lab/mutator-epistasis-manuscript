...
> For example, we find that AMSD is better-powered than QTL mapping when the number of simulated *de novo* mutations is variable across haplotypes (Figure {@fig:ihd_vs_qtl_power_variable}).
> We also note that many of the parameters used in our simulations are specific to the BXD population (e.g., number of haplotypes, expected allele frequencies at markers), and do not necessarily reflect the power of AMSD on other experimental populations. 
