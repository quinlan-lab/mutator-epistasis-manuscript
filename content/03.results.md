...
Given 100 haplotypes with an average of 500 *de novo* germline mutations each, IHD has approximately 80% power to detect a mutator allele that increases the C>T *de novo* mutation rate by as little as 10%. 
However, the approach only has about 10% power to detect a mutator of identical effect size that augments the C>A mutation rate, since C>A mutations are expected to make up a smaller fraction of all *de novo* germline mutations to begin with. 
...
Such interactions could manifest as loci associated with mutation spectrum variation
...
Finally, we queried a dataset of structural variants (SVs) identified via high-quality, long-read assembly of inbred laboratory strains [@doi:10.1016/j.xgen.2023.100291] and found 148 large insertions or deletions within the implicated interval on chromosome 6. 
