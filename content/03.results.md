...
At every locus, we use a $\chi^2$ test of independence to compare the aggregate mutation spectra of haplotypes that inherited either allele; the resulting $\chi^2$ statistic is therefore a measure of "distance" between the two spectra.
Using permutation tests, we then identify loci at which $\chi^2$ statistics are larger than what we'd expect by random chance.
...
Finally, as the IHD approach does not perform separate tests for every $k$-mer mutation type, it is better-powered to detect loci that affect mutation spectrum variation than traditional quantitative trait locus (QTL) mapping (Figure {@fig:ihd_vs_qtl_power}; Materials and Methods).
