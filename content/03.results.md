...
Our approach leverages the fact that mutator alleles often leave behind distinct and detectable impressions on the *mutation spectrum*, even if they increase the overall mutation rate by an imperceptible amount (Materials and Methods).
Given a population of haplotypes, we assume that a) each has been genotyped at a collection of biallelic loci and b) each harbors *de novo* mutations that have been classified by $k$-mer context (Figure @fig:distance-method).
At every locus, we use a $\chi^2$ test of indepdenence to compare the aggregate mutation spectrum of haplotypes that inherited either allele; the resulting $\chi^2$ statistic is therefore a measure of "distance" between the haplotypes' spectra.
