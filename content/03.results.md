...
This signal was specific to BXDs with *D* genotypes at the `rs52263933` locus, as we did not observe any new mutator loci (other than some residual signal from the *Mutyh* mutator locus) using those with *B* genotypes at `rs52263933` (Figure @fig:distance-results-conditioned).
...
We observed that C>A germline mutation fractions in BXDs with *D* alleles at both mutator loci were higher than in BXDs with *D* alleles at either locus alone (Figure {@fig:spectra-comparison}a and @fig:spectra-comparison-all). 
