...
#### Estimating confidence intervals around IHD peaks

If we identified an adjusted cosine distance peak on a particular chromosome, we used a bootstrap resampling approach [@PMID:8725246] to estimate a confidence interval around that peak.
...
Using those resampled spectra and genotypes, we performed an inter-haplotype distance scan on the chromosome of interest and recorded the position of the marker with the largest adjusted cosine distance value.
...
The BXD RILs were inbred for variable numbers of generations, and each BXD therefore accumulated a variable number of *de novo* germline mutations [@PMID:].
...
We also performed a series of simulations to assess the impact of mutator allele frequencies on the power of both inter-haplotype distance and QTL mapping.
In these simulatations, we simulated genotypes as described above, but subsequently altered the fraction of haplotypes with $1$ alleles at $s_i$ (that is, the simulated mutator locus) to be 10%, 25%, etc., rather than 50%.
