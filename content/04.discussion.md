...
Each of these genes fills a distinct role in the BER reponse to oxidative DNA damage, and thereby the prevention of C>A mutations [@PMID:28963982;@PMID:24732879].
...
Although we would expect *a priori* that *Ogg1* deficiency would lead to increased 8-oxoG accumulation and elevated C>A mutation rates, these lines of evidence suggest that p.Thr95Ala may not be highly deleterious on its own, and might only exert a detectable effect on the BER pathway when *Mutyh* function is also impaired.
It is challenging to predict the functional consequences of the *Ogg1* p.Thr95Ala variant, and as we discuss below, the p.Asp129Asn missense mutation in *Mbd4* is also a compelling candidate mutator allele.
